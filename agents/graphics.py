...
from agents.utils import remap, constrain
from agents.dqn import DQNAgent
# from agents.smdqn import SMDQNAgent
# from agents.safedqn import SafeDQNAgent
from agents.ppo import PPOAgent
from agents.spdqn import SPDQNAgent
...
class AgentGraphics(object):
    ...
    def display(cls, agent, agent_surface, sim_surface=None):
        ...
        if isinstance(agent, DQNAgent) or isinstance(agent, SPDQNAgent):
            # DQNGraphics.display(agent, agent_surface, sim_surface)
            pass
        elif isinstance(agent, PPOAgent):
            # PPOGraphics.display(agent, agent_surface, sim_surface)
            pass


class PPOGraphics(object):
    """
    ...
    def display(cls, agent, surface, sim_surface=None, display_text=True):
        ...
        if agent.previous_state is None:
            return
        action_values, action_distribution = agent.action_distribution(agent.previous_state)
    ...
    def display_vehicles_attention(cls, agent, sim_surface):
        ...
        try:
            ...
            if (not hasattr(cls, "state")) or (cls.state != state).any():
                ...
                cls.state = state
            #print("The value: ", list(cls.v_attention.values())) # Jinpeng

            if list(cls.v_attention.values()):
    ...
    def compute_vehicles_attention(cls, agent, state):
        ...
        attention = agent.actor_critic.actor.get_attention_matrix(state_t).squeeze(0).squeeze(1).detach().cpu().numpy() # head, other
        ego, others, mask = agent.actor_critic.actor.split_input(state_t)
        mask = mask.squeeze()
        for head in range(attention.shape[0]):
            attention[head, 1:] = np_softmax(attention[head, 1:])
        v_attention = {}
        ...
        for v_index in range(1, state.shape[0]): # Jinpeng, start from 1
            if mask[v_index]:
                continue
            v_position = {}
            for feature in ["x", "y"]:
...
class DQNGraphics(object):
    ...
    def compute_vehicles_attention(cls, agent, state):
        import torch
        state_t = torch.tensor([state], dtype=torch.float).to(agent.device)
        attention = agent.value_net.get_attention_matrix(state_t).squeeze(0).squeeze(1).detach().cpu().numpy() # head, all_entities
