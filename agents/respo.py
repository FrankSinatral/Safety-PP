...
def check_nan(tensor, name):
    if torch.isnan(tensor).any():
        print(f"[NaN ALERT] {name} contains NaN!")
        print(f"Stats: min={tensor.min().item()}, max={tensor.max().item()}, mean={tensor.mean().item()}")
        import sys
        sys.exit(1)
...
class RESPOAgent(AbstractStochasticAgent):
    def __init__(self, env, config=None):
        super(RESPOAgent, self).__init__(config)
        self.env = env
        ...
        size_model_config(self.env, self.config["vp"])
        self.device = DEVICE

        self.vp = model_factory(self.config["vp"])
        #print("Barrier Config: ", self.config["barrier"])
        self.vp = self.vp.to(self.device)
        self.vp_optimizer = Adam(self.vp.parameters(), self.config["vp_lr"])
    ...
    def default_config(cls):
        return dict(common=dict(type ="MultiLayerPerceptron"),
                    ...
                    vp = dict(type = "MultiLayerPerceptron"),
    ...
    def record(self, state, action, reward, next_state, done, end, info):
        ...
        if isinstance(info["cost"], list):
            self.buffer.costs.append(torch.as_tensor(np.array([info['cost'][0]]), dtype=torch.float32).to(self.device)) # cost
        else:
        ...
        if self.timestep % self.config["T_horizon"] == 0:
            loss, loss_vp, punsafe, entropy = self.update()
            self.writer.add_scalar("agent/loss", loss, self.timestep)
            self.writer.add_scalar("agent/loss_vp", loss_vp, self.timestep)
            self.writer.add_scalar("agent/punsafe", punsafe, self.timestep)
    ...
    def update(self):
        ...
        cost_deviation = torch.clamp(cur_cost - cost_limit, -10.0, 10.0)
        ...
        for _ in range(self.config["K_epochs"]):
            ...
            with torch.no_grad():
                pvs = self.vp(s)
                pvs_ = self.vp(s_prime)
            #prews = c # cost is 1.0 or 0
            pret = c.clone()
            
            for i in range(pret.shape[0]-2, -1, -1):
                if end_mask[i] and (not done_mask[i]):
                    pret[i] = max(pret[i], self.config["gamma"] * pvs_[i])
                else:
                    pret[i] = max(pret[i], self.config["gamma"] * (1-done_mask[i]) * pret[i+1])
            ...
            per_pret = pret[perm].clone()
            per_pvs = pvs[perm].clone()
            ...
            for i in range(optim_iter_num):
                ...
                prob_a = torch.clamp(prob_a, min=1e-8)
                check_nan(prob_a, "prob_a")
                ratio = torch.exp(torch.log(prob_a) - per_old_logprob_a[index])  # a/b == exp(log(a)-log(b))

                punsafe = per_pvs[index]
                psafe = 1 - punsafe

                surr1 = ratio * per_adv[index] * psafe
                surr2 = torch.clamp(ratio, 1 - self.config["clip_rate"], 1 + self.config["clip_rate"]) * per_adv[index] * psafe

                loss_rpi = torch.min(surr1, surr2)
                loss_cpi = ratio* per_cadv[index] * (penalty_item * psafe + punsafe)

                pi_objective = (loss_rpi - loss_cpi ) / (1+penalty_item)
                check_nan(loss_rpi, "loss_rpi")
                check_nan(loss_cpi, "loss_cpi")
                ...
                check_nan(loss, "total_loss")
                # take gradient step
                self.optimizer.zero_grad()
                loss.mean().backward()
                self.optimizer.step()

                new_pvs = self.vp(per_s[index])
                loss_vp = self.loss_function(new_pvs, per_pret[index])
                self.vp_optimizer.zero_grad()
                loss_vp.mean().backward()
                self.vp_optimizer.step()

        # clear buffer
        self.buffer.clear()
        return loss.mean().detach().cpu(), loss_vp.mean().detach().item(), punsafe.mean().item(), entropy.detach().cpu()

    def save(self, filename):
        state = {'state_dict': self.actor_critic.state_dict(),
                 'optimizer': self.optimizer.state_dict(),
                 'vp': self.vp.state_dict(),
    ...
    def load(self, filename):
        ...
        self.vp.load_state_dict(checkpoint['vp'])
