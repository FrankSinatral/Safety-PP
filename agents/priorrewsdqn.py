...
class PriorRewsDQNAgent(AbstractStochasticAgent):
    def __init__(self, env, config=None):
        super(PriorRewsDQNAgent, self).__init__(config)
        self.env = env
    ...
    def record(self, state, action, reward, next_state, done, end, info):
        ...
        # Single-agent setting
        if info['crashed']:
            reward = self.config['new_collision_reward']
        self.memory.push(state, action, reward, next_state, done, info)
        batch, weights, sample_idxs = self.sample_minibatch()
        if batch:
            loss, td_error, _ = self.compute_bellman_residual(batch, weights)
            self.step_optimizer(loss)
            self.memory.update_batch_priorities(sample_idxs, td_error.cpu().numpy())
    ...
    def compute_bellman_residual(self, batch, weights=None, target_state_action_value=None):
        ...
        # Compute loss
        td_error = torch.abs(state_action_values-target_state_action_value).detach()
        ...
        return loss, td_error, target_state_action_value
