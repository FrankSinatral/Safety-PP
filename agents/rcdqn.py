...
class RCDQNAgent(AbstractStochasticAgent):
    def __init__(self, env, config=None):
        super(RCDQNAgent, self).__init__(config)
        self.env = env
        ...
        self.lambd = self.config["lambd_init"]
        self.lambd_lr = self.config["lambd_lr"]
        self.lambd_update_interval = self.config["lambd_update_interval"]
        self.costs = []
        self.cost_lim = 0
        self.total_cost = 0
    ...
    def record(self, state, action, reward, next_state, done, end, info):
        ...
        if batch:
            ...
            if self.steps % self.lambd_update_interval == 0:
                if len(self.costs)>0:
                    self.lambd += self.lambd_lr * max(0, np.mean(self.costs) - self.cost_lim)
                self.costs = []
                self.writer.add_scalar("agent/lambd", self.lambd, self.steps)
        if end: self.total_cost = 0
