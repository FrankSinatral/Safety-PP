...
class SPDQN:
	"""
		This version: fix unsafe portion for phi buffer only
	"""
	...
	def get_action(self, state, deterministic=False):
		...
		with torch.no_grad():
			...
			if self.crisp:
				if self.args.dynamic_threshold:
			...
			if self.t_step > self.args.apply_phi_after:
		...
		if self.t_step % self.UPDATE_EVERY == 0:
			...
			if len(self.replay_buffer) > self.BATCH_SIZE:
				...
				for _ in range(self.NUPDATES):
					if self.args.for_both:
						experiences = self.replay_buffer.safety_sample()
					else:
	...
	def update(self, experiences):
		...
		if self.t_step % self.args.phi_update_step == 0:
			if not self.args.for_both:
