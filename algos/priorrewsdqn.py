...
class PriorRewsDQN:
	...
	def __init__(self, state_size, action_size, args):
		...
		self.replay_buffer = PrioritizedReplayBuffer(state_size, buffer_size=args.buffer_size,
	...
	def step(self, state, action, reward, next_state, done, cost, end):
		...
		if cost > 0:
			reward = self.args.penalty_reward
	...
	def update(self):
		states, actions, rewards, next_states, dones, costs, weights, sample_idxs=self.replay_buffer.sample()
		...
		td_error = torch.abs(q_expected - q_targets).detach()

		# Compute loss
		#loss = self.loss(q_expected, q_targets) 
		loss = torch.mean((q_expected- q_targets.detach())**2 * weights)
		...
		self.optimizer.step()
		self.replay_buffer.update_batch_priorities(sample_idxs, td_error.squeeze(-1).cpu().numpy())
