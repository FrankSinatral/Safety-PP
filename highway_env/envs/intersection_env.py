...
class IntersectionEnv(AbstractEnv):
    ...
    def _info(self, obs: np.ndarray, action: int) -> dict:
        ...
        info["cost"] = float(any(vehicle.crashed for vehicle in self.controlled_vehicles) \
                             or (self.config["offroad_terminal"] and not self.vehicle.on_road))
        info["feasible"] = any(self.has_arrived(vehicle) for vehicle in self.controlled_vehicles)
        info["infeasible"] = info["cost"] > 0
        info["success"] = all(self.has_arrived(vehicle) for vehicle in self.controlled_vehicles)
        return info
